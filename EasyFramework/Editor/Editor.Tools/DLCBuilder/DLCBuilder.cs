...
namespace EasyFramework.Editor
{
    ...
    public interface IDLCBuilderExtension : IEditorToolExtension
    {
        void OnExecuteBefore() { }
        void OnExecuteAfter() { }
    }

    public partial class DLCBuilder : EditorTool<DLCBuilder, IDLCBuilderExtension>
    {
        
        [MenuItem("EasyFramework/Tools/DLCBuilder - Build", priority = EasyFrameworkToolsSettings.DLCBuilder)]
        public static void MenuItem1()
        {
            DLCBuilder.Instance.Build();
        }
        ...
        public void Build()
        {
            ...
            var versionName = string.IsNullOrEmpty(settings.dlcVersionId) ? Version.version.ToString() : settings.dlcVersionId;
            ...
            if ((settings.buildOptions & EDLCOptions.DLC) > 0)
            {
                BuildDLC($"{outputDir}/{EDLCOptions.DLC}", sourceDirs);
            }
        ...
        private void BuildDLC(string outputDir, string[] sourceDirs)
        {
            ...
            try
            {
                foreach (var resFile in fileList)
                {
                    ...
                    var md5 = HashHelper.MD5File(resFile);
