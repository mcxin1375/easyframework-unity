...
namespace EasyFramework.Editor
{
    ...
    public class PlayerBuilder : EditorTool<PlayerBuilder>
    {
        ...
        public string[] GetBuildFiles()
        {
            ...
            var arr = AssetBundleBuilder.Instance.GetBuildFiles();
            if (arr?.Length > 0) files.AddRange(arr);
            arr = DataBuilder.Instance.GetBuildFiles();
            if (arr?.Length > 0) files.AddRange(arr);
            arr = HybridCLRBuilder.Instance.GetBuildFiles();
            if (arr?.Length > 0) files.AddRange(arr);
