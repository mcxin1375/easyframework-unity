...
namespace EasyFramework
{
    public static partial class AOTHelper
    {
        ...
        public static async Task CopyDirectoryAsync(string sourceDirectory, string destinationDirectory, bool compareDifferent, bool deleteNotExists, Action<string, int, int> copyAction, CancellationToken token)
        {
            await Task.Run(() => {
        ...
        public static void CopyFilesMD5(string[] files, string destinationDirectory, Action<string, float> progressAction = null)
        {
            ...
            {
                string toFile = $"{destinationDirectory}/{Path.GetFileName(file)}";
                if (!AOTHelper.MD5FileEqual(file, toFile)) File.Copy(file, toFile, true);
                progressAction?.Invoke(file, index++ / files.Length);
            }
        }
